...
def load_data():
    entitlements = pd.read_excel(r"D:\Python_Project\securecode\SOD_Ruleset.xlsx", sheet_name="ENTITLEMENT_MST")
    privileges = pd.read_excel(r"D:\Python_Project\securecode\XX_7_PVLGS_MASTER_RPT.xlsx", sheet_name=None)

    if isinstance(privileges, dict):  
        privileges = next(iter(privileges.values()))

    privilege_roles = pd.read_excel(r"D:\Python_Project\securecode\XX_6_PVLG_TO_ROLE_RELATION_RPT.xlsx")
    role_hierarchy = pd.read_excel(r"D:\Python_Project\securecode\XX_5_ROLE_TO_ROLE_HIER_RPT.xlsx")
    user_roles = pd.read_excel(r"D:\Python_Project\securecode\XX_3_USER_ROLE_MAPPING_RPT.xlsx")
    sod_rules = pd.read_excel(r"D:\Python_Project\securecode\SOD_Ruleset.xlsx", sheet_name="SOD_MASTER")
    user_details = pd.read_excel(r"D:\Python_Project\securecode\XX_2_USER_DETAILS_RPT.xlsx")
    role_details = pd.read_excel(r"D:\Python_Project\securecode\XX_4_ROLE_MASTER_DETAILS_RPT.xlsx")
